...
function GlassCard({ children, className = "", id }) {
  return (
    <div
      id={id}
      className={`rounded-[2rem] border border-white/70 bg-white/76 shadow-[0_24px_90px_rgba(15,23,42,0.32)] backdrop-blur-3xl ${className}`}
    >
      {children}
    </div>
  );
...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-slate-950 bg-[url('/backgrounds/rain-window.jpg')] bg-cover bg-center bg-fixed text-slate-950">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12">
        <section className="mb-8 grid gap-6 lg:grid-cols-[1fr_360px]">
          <GlassCard className="relative overflow-hidden p-6 md:p-10">
            ...
            <div className="mt-7 flex flex-wrap gap-3 text-sm font-bold">
              <span className="rounded-full border border-white/70 bg-white/74 px-4 py-2 text-slate-700 shadow-md backdrop-blur-xl">дом 2 смотреть</span>
              <span className="rounded-full border border-white/70 bg-white/74 px-4 py-2 text-slate-700 shadow-md backdrop-blur-xl">дом 2 сегодняшний выпуск</span>
              <span className="rounded-full border border-white/70 bg-white/74 px-4 py-2 text-slate-700 shadow-md backdrop-blur-xl">дом 2 стримы</span>
            </div>
          </GlassCard>
          ...
          <GlassCard className="overflow-hidden p-3">
            <div className="relative min-h-[360px] overflow-hidden rounded-[1.65rem] bg-slate-950/20 shadow-xl md:min-h-[430px]">
              ...
              <div className="absolute left-4 top-4 rounded-full border border-white/70 bg-white/90 px-3 py-1 text-xs font-black text-slate-800 shadow-lg backdrop-blur-xl">
                фото
              </div>
            ...
            <div className="space-y-3">
              {archiveItems.map((item) => (
                <a
                  ...
                  className="group block overflow-hidden rounded-3xl border border-white/60 bg-white/72 p-2 shadow-md backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-white/86 hover:shadow-xl"
                >
        ...
        <section id="news" className="mt-8">
          <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
            <div>
              <div className="mb-2 text-sm font-black uppercase tracking-[0.22em] text-slate-600">новости проекта</div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {newsItems.map((item) => (
              <GlassCard key={item.title} className="overflow-hidden p-3">
                <div className="relative overflow-hidden rounded-[1.4rem] bg-white/76 shadow-lg">
                  <img
                    src={item.image}
                    alt={item.title}
                    className="aspect-[4/5] w-full object-cover object-center"
                  />
                  <div className="absolute left-3 top-3 rounded-full border border-white/70 bg-white/90 px-3 py-1 text-xs font-black text-slate-800 shadow-lg backdrop-blur-xl">
                    {item.tag}
                  </div>
                ...
                <div className="p-3">
                  <div className="mb-2 text-xs font-black uppercase tracking-[0.18em] text-slate-600">
      ...
      <footer id="contacts" className="relative z-10 border-t border-white/40 bg-white/20 px-4 py-8 text-center text-sm text-slate-600 backdrop-blur-2xl md:px-6">
