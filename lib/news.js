...
import { getTelegramNews, CHANNELS } from "./telegram.js";
...
/**
 * Лента новостей.
 * Сначала свежее из Telegram, затем ручной архив.
 */
export async function getNews() {
  ...
  try {
    ...
    // Telegram недоступен — сайт продолжает работать на ручных новостях.
    CHANNELS.forEach((c) => {
      byChannel[c.key] = [];
    });
  }
 ...
 * Если канал молчит — добираем из общей ленты, чтобы не было дыр.
 */
...
/** Находит новость по slug среди ручных. */
export function findManual(slug) {
  const item = manualNews.find((n) => n.href === `/news/${slug}`);
  return item ? normalizeManual(item) : null;
}
